class MakerFlowEngine {
  ...
  setupSpeech() {
    ...
    if (SR) {
      ...
      this.el.gateFallbackBtn.style.display = "block";
    }
  }
  ...
  syncModeButtons() {
    const l = `MODE: ${this.state.mode.toUpperCase()}`;
    this.el.modeBtnGate.textContent = l;
    this.el.modeBtnApp.textContent = l;
  }
  ...
  armGatekeeper() {
    ...
    const tryStart = () => {
      try {
        this.el.startCircle.classList.add("listening");
    ...
    this.gateRecognition.onresult = (e) => {
      ...
      if (said.includes("start") || said.includes("los")) {
        this.startApp();
      }
    };

    this.gateRecognition.onerror = this.gateRecognition.onend = () => {
      this.el.startCircle.classList.remove("listening");
      if (this.el.gate.classList.contains("hidden")) return;
  ...
  toast(msg) {
    ...
    setTimeout(() => this.el.feedback.textContent = "", 1500);
  }
}
